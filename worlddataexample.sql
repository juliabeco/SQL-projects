...
/*MOSTRAR TODAS LAS VARIABLES DE LA TABLA countries*/
SELECT * 
FROM countries;

/*CONTEO DE TOTAL DE PAISES*/
SELECT COUNT(name) AS "TOTAL COUNTRIES" 
FROM countries;

/*LISTAR PAISES QUE TENGAN LOS CARACTERES "AY" EN ALGUNA PARTE DEL NOMBRE*/
SELECT name, fertility_rate
FROM countries
WHERE LOWER(name) LIKE "%ay%"; 

/*ESTADISTICAS GENERALES, MEDIA, MINIMA Y MAXIMA DE POBLACION*/
SELECT ROUND(AVG(population)) AS "AVERAGE POPULATION", 
MIN(population) AS MIN, 
MAX(population)AS MAX 
FROM countries;

/*5 PAISES CON MENOS POBLACION*/
...
/*10 PAISES CON MAYOR POBLACION*/
SELECT name, population 
FROM countries
ORDER BY population DESC
LIMIT 10;

/*ESTADISTICAS GENERALES DE % DE CAMBIO EN UN AÑO Y CAMBIO POBLACIONAL (VALORES ABSOLUTOS)*/

SELECT ROUND(AVG(percent_one_year_change),2) AS "AVG % 1 YEAR CHANGE", 
MIN(percent_one_year_change) AS MIN, 
MAX(percent_one_year_change) AS MAX,
AVG(population_change) AS "AVG POPULATION CHANGE", 
MIN(population_change) AS "MIN POP CHANGE", 
MAX(population_change) AS "MAX POP CHANGE"
FROM countries;

/*EL TOP 3 DE PAISES QUE PRESENTARON  EN UN AÑO EL % DE CAMBIO POBLACIONAL NEGATIVO MÁS ALTO*/
...
/*EL TOP 3 DE PAISES QUE PRESENTARON EN UN AÑO EL % DE CAMBIO POBLACIONAL POSITIVO MÁS ALTO*/
SELECT name, percent_one_year_change
FROM countries 
GROUP BY name 
...
/*ESTADISTICAS DESCRIPTIVA PARA DESIDAD POR KM2 Y AREA KM2*/
SELECT AVG(density_per_sq_km) AS "DENSITY PER KM2",
MIN(density_per_sq_km) AS "MIN DENSITY",
MAX(density_per_sq_km) AS "MAX DENSITY",
AVG(area_sq_km) AS "AREA PER KM2",
MIN(area_sq_km) AS "MIN AREA",
MAX(area_sq_km) AS "MAX AREA"
FROM countries;

/*ESTADISTICAS DESCRIPTIVAS DE MEDIANA DE EDAD*/
SELECT AVG(median_age) AS "MEDIAN AGE", MIN(median_age) AS MIN, MAX(median_age) AS MAX FROM countries;

/*CATEGORIZAR MEDIANA DE EDAD EN 3 GRUPOS*/
...
END AS "MEDIAN AGE OF ALL COUNTRIES"
FROM countries;

/*MOSTRAR EL TOP 15 ASCENDENTE DE LOS PAISES CON MEDIANA DE EDAD DE LA SEGUNDA CATEGORIA*/
SELECT name, median_age,
    CASE 
        WHEN median_age < 20 THEN "LESS 20 MEDIAN"
        WHEN median_age < 40 THEN "BTW 20-40 MEDIAN"
    ...
    END AS "MEDIAN AGE OF ALL COUNTRIES"
FROM countries
WHERE median_age >= 20 AND median_age < 40
ORDER BY median_age ASC
LIMIT 15;

/*ESTADISTICAS DESCRIPTIVAS DEL PORCENTAJE DE POBLACION MUNDIAL*/
SELECT ROUND(AVG(percent_of_world_pop),3) AS "% WORLD POP", 
MIN(percent_of_world_pop) AS MIN, 
MAX(percent_of_world_pop) AS MAX 
FROM countries;

/*LISTAR NOMBRE Y POBLACION TOTAL DE LOS PAISES CON MAS DE 100MILLONES DE HAB EN ORDEN DESCENDENTE*/
SELECT name, population AS "total_population" 
FROM countries 
GROUP BY name
HAVING population > 100000000 
ORDER BY population DESC; 

/*CONTAR CUANTOS PAISES TIENEN MAS DE 100MILLONES DE HABITANTES*/
SELECT COUNT(*) AS "N_countries_pop_ABOVE_100M" 
FROM countries 
WHERE population > 100000000;

/*CATEGORIZAR EN 3 EL % DE POBLACION MUNDIAL*/
SELECT name, percent_of_world_pop,
CASE 
    WHEN percent_of_world_pop < 1.5 THEN "LESS 1.5%"
    WHEN percent_of_world_pop < 3 THEN "BTW 1.5-3%"
    ELSE "MORE THAN 3%"
END AS "% OF WORLDS POP"
FROM countries;

/*LISTAR PAIS Y SU POBLACION CUANDO EL % DE POBLACION MUNDIAL SEA MAYOR A 3 Y TENGA UN AREA EN KM2 MAYOR A 100000*/
SELECT name, population 
FROM countries 
WHERE percent_of_world_pop> 3 AND area_sq_km >100000;

/*LISTAR PAIS Y SU POBLACION CUANDO EL % DE POBLACION MUNDIAL SEA MAYOR A 5 Y TENGA UN AREA EN KM2 MAYOR A 2000000*/
SELECT name, population 
FROM countries 
WHERE percent_of_world_pop> 5 OR area_sq_km >2000000;
